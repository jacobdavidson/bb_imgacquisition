...
void SharedMemory::run()
{
    ...
    while (true)
    {
        ...
        std::shared_ptr<ImageBuffer> imgptr = _Buffer->pop();
...
boost::interprocess::interprocess_mutex* SharedMemory::createSharedMemory(key_t* key,
                                                                          int*   shmid,
                                                                          char** data,
                                                                          int    id)
{
    ...
    *data = static_cast<char*>(shmat(*shmid, (void*) 0, 0));
    ...
    boost::interprocess::interprocess_mutex* mutex = new ((*data) + lockpos)
