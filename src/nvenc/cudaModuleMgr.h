...
// We have a C++ and a C version so that developers can choose to use either depending on their
// preferences
...
extern "C" bool modInitCtx(sCtxModule* mCtx,
                           const char* filename,
...
public:
    ...
    CUmoduleManager(const char* filename_module,
                    const char* exec_path,
                    int         nKernels,
                    int         nGlobalMem,
                    int         nTexRef);
