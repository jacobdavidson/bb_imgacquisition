...
NVENCSTATUS CNvHWEncoder::NvEncGetEncodeProfileGUIDCount(GUID      encodeGUID,
                                                         uint32_t* encodeProfileGUIDCount)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;

    nvStatus = m_pEncodeAPI->nvEncGetEncodeProfileGUIDCount(m_hEncoder,
                                                            encodeGUID,
                                                     ...
                                                     uint32_t* GUIDCount)
{
    ...
    nvStatus = m_pEncodeAPI->nvEncGetEncodeProfileGUIDs(m_hEncoder,
                                                        encodeGUID,
                                                        profileGUIDs,
                                                        guidArraySize,
                                                        GUIDCount);
...
NVENCSTATUS CNvHWEncoder::NvEncGetEncodeGUIDs(GUID*     GUIDs,
                                              uint32_t  guidArraySize,
                                              uint32_t* GUIDCount)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
                                               ...
                                               uint32_t*             inputFmtCount)
{
    ...
    nvStatus = m_pEncodeAPI->nvEncGetInputFormats(m_hEncoder,
                                                  encodeGUID,
                                                  inputFmts,
                                                  inputFmtArraySize,
                                                  inputFmtCount);
...
NVENCSTATUS CNvHWEncoder::NvEncGetEncodeCaps(GUID               encodeGUID,
...
NVENCSTATUS CNvHWEncoder::NvEncGetEncodePresetCount(GUID      encodeGUID,
                                                    uint32_t* encodePresetGUIDCount)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;

    nvStatus = m_pEncodeAPI->nvEncGetEncodePresetCount(m_hEncoder,
                                                       encodeGUID,
                                                    ...
                                                    uint32_t* encodePresetGUIDCount)
{
    ...
    nvStatus = m_pEncodeAPI->nvEncGetEncodePresetGUIDs(m_hEncoder,
                                                       encodeGUID,
                                                       presetGUIDs,
                                                       guidArraySize,
                                                       encodePresetGUIDCount);
...
NVENCSTATUS CNvHWEncoder::NvEncGetEncodePresetConfig(GUID                  encodeGUID,
                                                     GUID                  presetGUID,
                                                     NV_ENC_PRESET_CONFIG* presetConfig)
{
    ...
    nvStatus =
        m_pEncodeAPI->nvEncGetEncodePresetConfig(m_hEncoder, encodeGUID, presetGUID, presetConfig);
    if (nvStatus != NV_ENC_SUCCESS)
    {
        assert(0);
    }
...
NVENCSTATUS CNvHWEncoder::NvEncCreateInputBuffer(uint32_t width,
                                                 uint32_t height,
                                                 void**   inputBuffer,
                                                 uint32_t isYuv444)
{
    ...
    createInputBufferParams.bufferFmt  = isYuv444 ? NV_ENC_BUFFER_FORMAT_YUV444_PL
...
NVENCSTATUS CNvHWEncoder::NvRunMotionEstimationOnly(EncodeBuffer* pEncodeBuffer[2],
                                                    MEOnlyConfig* pMEOnly)
{
    ...
    if (m_fOutput)
    {
        ...
        {
            fprintf(m_fOutput,
                    "block = %d, mb_type = %d, partitionType = %d, MV[0].x = %d, MV[0].y = "
                    "%d, "
                    "MV[1].x = %d, MV[1].y = %d, MV[2].x = %d, MV[2].y = %d, MV[3].x = %d, "
                    "MV[3].y = %d, cost=%d ",
                    i,
                    outputMV[i].mb_type,
...
NVENCSTATUS CNvHWEncoder::NvEncLockInputBuffer(void*     inputBuffer,
                                               void**    bufferDataPtr,
                                               uint32_t* pitch)
{
    NVENCSTATUS              nvStatus = NV_ENC_SUCCESS;
...
NVENCSTATUS CNvHWEncoder::NvEncGetSequenceParams(
...
NVENCSTATUS CNvHWEncoder::NvEncInvalidateRefFrames(const NvEncPictureCommand* pEncPicCommand)
{
    ...
    {
        nvStatus = m_pEncodeAPI->nvEncInvalidateRefFrames(m_hEncoder,
                                                          pEncPicCommand->refFrameNumbers[i]);
    }
...
NVENCSTATUS CNvHWEncoder::NvEncReconfigureEncoder(const NvEncPictureCommand* pEncPicCommand)
{
    ...
    if (pEncPicCommand->bBitrateChangePending || pEncPicCommand->bResolutionChangePending)
    {
        ...
        if (pEncPicCommand->bBitrateChangePending)
        {
            ...
            m_stEncodeConfig.rcParams.vbvBufferSize =
                pEncPicCommand->newVBVSize != 0
                    ...
                    : (pEncPicCommand->newBitrate * m_stCreateEncodeParams.frameRateDen) /
        ...
        memcpy(&stReconfigParams.reInitEncodeParams,
               &m_stCreateEncodeParams,
               sizeof(m_stCreateEncodeParams));
...
NVENCSTATUS CNvHWEncoder::ValidateEncodeGUID(GUID inputCodecGuid)
{
    ...
    nvStatus            = m_pEncodeAPI->nvEncGetEncodeGUIDs(m_hEncoder,
                                                 encodeGUIDArray,
                                                 encodeGUIDCount,
                                                 &encodeGUIDArraySize);
    if (nvStatus != NV_ENC_SUCCESS)
...
NVENCSTATUS CNvHWEncoder::CreateEncoder(const EncodeConfig* pEncCfg)
{
    ...
    GUID inputCodecGUID = pEncCfg->codec == NV_ENC_H264 ? NV_ENC_CODEC_H264_GUID
    ...
    nvStatus = m_pEncodeAPI->nvEncGetEncodePresetConfig(m_hEncoder,
    ...
    if (pEncCfg->rcMode == 0)
    {
        m_stEncodeConfig.rcParams.constQP.qpInterP = pEncCfg->presetGUID ==
                                                             NV_ENC_PRESET_LOSSLESS_HP_GUID
                                                         ? 0
                                                         : pEncCfg->qp;
        m_stEncodeConfig.rcParams.constQP.qpInterB = pEncCfg->presetGUID ==
                                                             NV_ENC_PRESET_LOSSLESS_HP_GUID
                                                         ? 0
                                                         : pEncCfg->qp;
        m_stEncodeConfig.rcParams.constQP.qpIntra = pEncCfg->presetGUID ==
                                                            NV_ENC_PRESET_LOSSLESS_HP_GUID
                                                        ? 0
                                                        : pEncCfg->qp;
    }
    ...
    if (pEncCfg->intraRefreshEnableFlag)
    {
        if (pEncCfg->codec == NV_ENC_HEVC)
        {
            ...
            m_stEncodeConfig.encodeCodecConfig.hevcConfig.intraRefreshPeriod =
                pEncCfg->intraRefreshPeriod;
            m_stEncodeConfig.encodeCodecConfig.hevcConfig.intraRefreshCnt =
                pEncCfg->intraRefreshDuration;
        }
        else
        {
            m_stEncodeConfig.encodeCodecConfig.h264Config.enableIntraRefresh = 1;
            m_stEncodeConfig.encodeCodecConfig.h264Config.intraRefreshPeriod =
                pEncCfg->intraRefreshPeriod;
            m_stEncodeConfig.encodeCodecConfig.h264Config.intraRefreshCnt =
                pEncCfg->intraRefreshDuration;
        }
    }
    ...
    if (pEncCfg->enableMEOnly == 1 || pEncCfg->enableMEOnly == 2)
    {
        ...
        int meonlyMode                          = 0;
        nvStatus                                = m_pEncodeAPI->nvEncGetEncodeCaps(m_hEncoder,
                                                    m_stCreateEncodeParams.encodeGUID,
                                                    &stCapsParam,
                                                    &meonlyMode);
        if (nvStatus != NV_ENC_SUCCESS)
...
NVENCSTATUS CNvHWEncoder::ProcessOutput(const EncodeBuffer* pEncodeBuffer)
{
    ...
    if (nvStatus == NV_ENC_SUCCESS)
    {
        fwrite(lockBitstreamData.bitstreamBufferPtr,
               1,
               lockBitstreamData.bitstreamSizeInBytes,
               m_fOutput);
        nvStatus = m_pEncodeAPI->nvEncUnlockBitstream(m_hEncoder,
                                           ...
                                           uint32_t             qpDeltaMapArraySize)
{
    ...
    if (encPicCommand)
    {
        ...
        if (encPicCommand->bForceIntraRefresh)
        {
            ...
            {
                encPicParams.codecPicParams.h264PicParams.forceIntraRefreshWithFrameCnt =
                    encPicCommand->intraRefreshDuration;
            }
        }
    }
...
NVENCSTATUS CNvHWEncoder::ParseArguments(EncodeConfig* encodeConfig, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (stricmp(argv[i], "-preloadedFrameCount") == 0)
        {
            ...
            {
                PRINTERR(
                    "invalid preloadedFrameQueueSize value = %d (permissive value 2 "
                    "and above)\n",
